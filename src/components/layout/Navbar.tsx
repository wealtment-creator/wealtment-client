...
export function Navbar() {
  ...
  const linkCls = (href: string) =>
    `text-xs font-semibold tracking-widest uppercase transition-colors px-1.5 py-1 rounded md:px-3 py-1.5 ${
      isActive(href)
    ...
    <header className="fixed top-0 inset-x-0 z-50 border-b border-[var(--border)] bg-[rgba(10,22,40,0.96)] backdrop-blur-xl">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between gap-4">
        ...
        <Link href="/" className="flex items-center gap-2 shrink-0">
          <div className="w-8 h-8 rounded-lg overflow-hidden hidden md:block">
            <Image src={logo} alt="wealtment" width={32} height={32} priority className="object-cover w-full h-full" />
          </div>
          <span className="font-display font-bold text-xs tracking-wider text-gold-grad  sm:block">
            Wealtment
          </span>
        </Link>
        ...
        <div className="bg-[var(--bg-2)] border-t border-[var(--border)] px-4 py-4 flex flex-col gap-1">
          {user ? (
            <>
              ...
              <MobLink href="/contact"  label="Contact"     close={() => setOpen(false)} />
            ...
            <>
              ...
              {PUBLIC_LINKS.map((l) => (
<span key={l.label} className="py-1">


{
// l.label!=="Affiliates"  && l.label!=="Plans" 
